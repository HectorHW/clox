        ...
        switch (OBJ_TYPE(callee)){
            ...
            case OBJ_CLASS: {
                ...
                if(tableGet(&klass->methods, vm.initString, &initializer)){
                    ...
                    //passing arguments to class without initializer makes no sense
            ...
            case OBJ_BOUND_METHOD: {
                ...
                vm.stackTop[-argCount-1] = bound->receiver; //set `this`
                return call(bound->method, argCount);
            }

            default:
                break; // object is not callable function
        }
    }
...
((constant_variable) = frame->closure->function->chunk.constants.values[*frame->ip + (*(frame->ip+1)<<8) + (*(frame->ip+2)<<16)], frame->ip+=3)
    ...
    for(;;){
        ...
        switch (instruction=READ_BYTE()) {
            ...
            case OP_GET_PROPERTY: {
                ...
                Value value;
                if(tableGet(&instance->fields, name, &value)){ //try to get object field first
                    pop();
                    push(value);
                    break;
                }

                //if field lookup fails, get class method and bind it
            ...
            case OP_RETURN: {
                ...
                vm.frameCount--;
                if(vm.frameCount==0) { //return from top level code
                    pop();
                    return INTERPRET_OK;
                }
