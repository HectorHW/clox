...
typedef struct Compiler {
    ...
    FunctionType type; // are we compiling top-level code or function body
...
static void errorAt(Token* token, const char* message){
    ...
    if(token->type==TOKEN_EOF){
        ...
        //nothing here - TOKEN_ERROR is artificial
    }else{
...
static void advance() {
    ...
    for(;;) {
        ...
        errorAtCurrent(parser.current.start); //may report multiple times
    }
}
...
static void emitBytes2(uint8_t byte1, uint8_t byte2){
...
static int makeConstant(Value value){
    int constant_addr = addConstant(currentChunk(), value);
    return constant_addr;
}

static void emitConstant(Value value){
    int addr = makeConstant(value);

    if(addr<=UINT8_MAX){
        emitBytes2(OP_CONSTANT, (uint8_t)addr);
    }else{

        emitByte(OP_CONSTANT_LONG);
        emitBytes3( //address as 3 bytes (might change)
                addr&     0xff,
                (addr&  0xff00)>>8,
                (addr&0xff0000)>>16
                );
...
static void initCompiler(Compiler* compiler, FunctionType type){
    ...
    compiler->function = NULL; // for GC later
    compiler->type = type;
...
static void addLocal(Token name) {
    ...
    Local *local = &current->locals[current->localCount++];
    local->name = name;
    local->depth = -1; //variable is uninitialized by default
    local->isCaptured = false;
}
...
static void declareVariable(){
    ...
    for(int i = current->localCount-1; i>=0;i--) {
        ...
        if(identifiersEqual(name, &local->name)){
            error("Variable with this name already exists in this scope.");
        }
    }
...
static void binary(bool canAssign){
    ...
    switch (operatorType) {
        ...
        case TOKEN_BANG_EQUAL: emitBytes2(OP_EQUAL, OP_NOT); break; // != ~~~ !(==)
        case TOKEN_EQUAL_EQUAL : emitByte(OP_EQUAL); break;
        case TOKEN_GREATER: emitByte(OP_GREATER); break;
        case TOKEN_GREATER_EQUAL: emitBytes2(OP_LESS, OP_NOT); break; // >= ~~~ !(<)
        case TOKEN_LESS: emitByte(OP_LESS); break;
        case TOKEN_LESS_EQUAL: emitBytes2(OP_GREATER, OP_NOT); break; // <= ~~~ !(>)
        default:
            return; //unreachable
    }
...
static void namedVariable(Token name, bool canAssign){
    ...
    if(canAssign && match(TOKEN_EQUAL)){
        expression();
        emitBytes2(setOp, (uint8_t)arg);
    }else{
        emitBytes2(getOp, arg);
    }
...
static void super_(bool canAssign){
    ...
    if(match(TOKEN_LEFT_PAREN)){
        ...
        emitBytes3(OP_SUPER_INVOKE, name, argCount);
    }else{
        namedVariable(syntheticToken("super"), false);
        emitBytes2(OP_GET_SUPER, name);
    }
...
static void function(FunctionType type){
    ...
    ObjFunction *function_ = endCompiler();
    emitBytes2(OP_CLOSURE, makeConstant(OBJ_VAL(function_)));
...
static void classDeclaration(){
    ...
    if (match(TOKEN_LESS)){
        ...
        namedVariable(className, false); //current class
        emitByte(OP_INHERIT);
...
static void varDeclaration(){
    ...
    if(match(TOKEN_EQUAL)){
        ...
        emitByte(OP_NIL); //store nil if no assigner
    }
